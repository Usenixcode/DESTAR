...
#between different neural network designs.
DEFAULT_MODEL_TYPE = 'linear'   #(linear|categorical|rnn|imu|behavior|3d|localizer|latent)
BATCH_SIZE = 128                #how many records to use when doing one pass of gradient decent.
...
PRUNE_CNN = False               #This will remove weights from model. The primary goal is to increase performance.
...
#be used to copy the final model to the vehicle. If not using this option, no need to set these.
...
#LED
HAVE_RGB_LED = False            #if there is an RGB LED
LED_INVERT = False              #COMMON ANODE? Some RGB LED use common anode.
