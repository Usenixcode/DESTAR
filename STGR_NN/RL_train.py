...
class RLTrainer:
    ...
    def get_reward(self, node_features, adj_matrix, edge_attributes_seq, true_x, true_y, true_yaw, true_vx, true_vy, true_yawrate, true_ax, true_ay):
        ...
        time_diff_loss = self.compute_time_diff_loss(predicted_x, predicted_y, predicted_vx, predicted_vy, predicted_ax, predicted_ay, true_x, true_y, true_vx, true_vy, true_ax, true_ay)
        total_loss = (loss_x + loss_y + loss_vx + loss_vy + loss_yaw + loss_yawrate + loss_ax + loss_ay + time_diff_loss) / 14
    ...
    def compute_time_diff_loss(self, predicted_x, predicted_y, predicted_vx, predicted_vy, predicted_ax, predicted_ay, true_x, true_y, true_vx, true_vy, true_ax, true_ay):
        time_diff_x = torch.abs(predicted_x[:, 1:, :] - predicted_x[:, :-1, :]).mean()
        time_diff_y = torch.abs(predicted_y[:, 1:, :] - predicted_y[:, :-1, :]).mean()
        time_diff_vx = torch.abs(predicted_vx[:, 1:, :] - predicted_vx[:, :-1, :]).mean()
        time_diff_vy = torch.abs(predicted_vy[:, 1:, :] - predicted_vy[:, :-1, :]).mean()
        time_diff_ax = torch.abs(predicted_ax[:, 1:, :] - predicted_ax[:, :-1, :]).mean()
        time_diff_ay = torch.abs(predicted_ay[:, 1:, :] - predicted_ay[:, :-1, :]).mean()

        trend_penalty_x = torch.mean((torch.sign(predicted_x[:, 1:, :] - predicted_x[:, :-1, :]) * torch.sign(true_x[:, 1:, :] - true_x[:, :-1, :]) < 0).float())
        trend_penalty_y = torch.mean((torch.sign(predicted_x[:, 1:, :] - predicted_x[:, :-1, :]) * torch.sign(true_x[:, 1:, :] - true_x[:, :-1, :]) < 0).float())
        trend_penalty_vx = torch.mean((torch.sign(predicted_x[:, 1:, :] - predicted_x[:, :-1, :]) * torch.sign(true_x[:, 1:, :] - true_x[:, :-1, :]) < 0).float())
        trend_penalty_vy = torch.mean((torch.sign(predicted_x[:, 1:, :] - predicted_x[:, :-1, :]) * torch.sign(true_x[:, 1:, :] - true_x[:, :-1, :]) < 0).float())
        trend_penalty_ax = torch.mean((torch.sign(predicted_x[:, 1:, :] - predicted_x[:, :-1, :]) * torch.sign(true_x[:, 1:, :] - true_x[:, :-1, :]) < 0).float())
        trend_penalty_ay = torch.mean((torch.sign(predicted_x[:, 1:, :] - predicted_x[:, :-1, :]) * torch.sign(true_x[:, 1:, :] - true_x[:, :-1, :]) < 0).float())
        

        total_trend_penalty = (trend_penalty_x + trend_penalty_y + trend_penalty_vx + trend_penalty_vy + trend_penalty_ax + trend_penalty_ay) * 0.5
        
        time_diff_loss = total_trend_penalty
